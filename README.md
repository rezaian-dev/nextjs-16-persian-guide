...
خروجی‌های `.next/` و `out/` تولیدی‌اند و در مخزن نگهداری نمی‌شوند. پوشهٔ `docs/` حذف شده است؛ انتشار GitHub Pages با workflow فایل `.github/workflows/pages.yml` انجام می‌شود: پس از هر push روی `master`، تایپ‌ها بررسی و خروجی `out/` ساخته و منتشر می‌شود. در Settings → Pages، گزینهٔ Source باید GitHub Actions باشد؛ هیچ خروجی بیلدی در مخزن ثبت نمی‌شود. برای Node/Vercel از `npm run build` و `npm start` استفاده کنید.
