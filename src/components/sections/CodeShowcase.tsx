...
const CHIPS = ["use cache", "cacheTag", "Suspense", "Server Action", "PPR", "Turbopack"];

const LINES = [
  { text: "// کش صریح — در نسخهٔ ۱۶ همه‌چیز پیش‌فرض پویاست", cls: "text-faint" },
  { text: "async function getStats() {", cls: "text-sky-300" },
  { text: "  \"use cache\";", cls: "text-primary-soft" },
  { text: "  cacheTag(\"stats\");", cls: "text-sky-300" },
  { text: "  return db.stats.findMany();", cls: "text-sky-300" },
  { text: "}", cls: "text-sky-300" },
  { text: "", cls: "" },
  { text: "// صفحه روی سرور می‌ماند؛ کلاینت فقط تعامل", cls: "text-faint" },
  { text: "export default async function Page() {", cls: "text-sky-300" },
  { text: "  return (", cls: "text-sky-300" },
  { text: "    <Suspense fallback={<Skeleton />}>", cls: "text-violet-300" },
  { text: "      <Stats data={getStats()} />", cls: "text-violet-300" },
  { text: "    </Suspense>", cls: "text-violet-300" },
  { text: "  );", cls: "text-sky-300" },
  { text: "}", cls: "text-sky-300" },
];
...
کد <span className="grad-text">Production</span>، نه اسباب‌بازی
              ...
              <p className="mt-3.5 text-[15px] leading-8 text-muted-foreground">
                هر الگو با کد کامل و قابل اجرا می‌آید؛ ۲۱۶ پنجرهٔ کد در طول کتاب.
              </p>
            ...
            <div dir="ltr" className="overflow-hidden rounded-2xl border border-input bg-card shadow-[0_40px_110px_-30px_rgb(6_10_24_/_0.65)]">
              <div className="flex items-center gap-1.5 border-b border-border bg-secondary/40 px-4 py-3 font-mono text-[11.5px] text-muted-foreground">
                ...
                <span className="ms-2">app/dashboard/page.tsx</span>
              </div>
