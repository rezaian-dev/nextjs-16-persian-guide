    ...
    <section className="flex min-h-svh items-center pb-16 pt-32 md:pt-40" id="top">
      <div className="container grid items-center gap-12 lg:grid-cols-[1.06fr_0.8fr] lg:gap-16">
        <m.div variants={container} initial="hidden" animate="show">
          <m.div variants={item}>
            <Badge variant="outline" className="gap-2 px-4 py-2 text-[13px] text-sub">
              ...
              ویرایش ۲۰۲۶ · Next.js 16 · رایگان و متن‌باز
            </Badge>
          </m.div>
          ...
          >
            Next.js <span className="grad-text">16</span>
          </m.h1>

          <m.p variants={item} className="mt-5 max-w-xl text-lg font-semibold leading-9 text-sub md:text-xl">
            از App Router و Server Components تا معماری، امنیت و استقرار Production.‏
          </m.p>

          <m.p variants={item} className="mt-3 max-w-xl text-[15px] leading-7 text-muted-foreground">
            یاد می‌گیری داده کجا خوانده شود، چه چیزی کش شود و مرز Server و Client کجاست — با{" "}
            ...
            ، Server Actions و PPR‏. ۳۷ فصل، ۱۷۸ صفحه.
          </m.p>
        ...
        <div className="relative mx-auto w-full max-w-[360px]">
          <div
            className="absolute inset-0 -z-10 rounded-full bg-[radial-gradient(circle,rgb(56_189_248_/_0.26),rgb(86_200_250_/_0.13)_45%,transparent_70%)] blur-[30px]"
            aria-hidden="true"
          />

          <div>
            <div className="rounded-[22px] border border-input shadow-[0_50px_130px_-30px_rgb(6_10_24_/_0.7),0_0_90px_-30px_rgb(56_189_248_/_0.4)] motion-reduce:animate-none">
              {/* Lossless PDF-derived assets; keep text-bearing artwork free of transforms. */}
              <img
                src={asset("/cover-hero.webp")}
                srcSet={`${asset("/cover-hero-480.webp")} 480w, ${asset("/cover-hero-720.webp")} 720w, ${asset("/cover-hero.webp")} 1080w`}
                sizes="(max-width: 400px) calc(100vw - 40px), 360px"
                alt="جلد مرجع فارسی Next.js 16"
                width={1080}
                height={1529}
                fetchPriority="high"
                decoding="async"
