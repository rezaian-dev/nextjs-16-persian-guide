...
"""The round author avatar for the README footer, cut from the portrait the cover embeds.
    ...
    python3 src/tools/make_avatar.py            # write docs/assets/avatar.png
    python3 src/tools/make_avatar.py --check    # fail if the shipped file is stale

`author.png` is the cover's embedded portrait, byte for byte — 560 × 560 px of real detail, and
it must stay that way, so nothing here touches it. The README footer is a different job: a small
round headshot. Painting the 560 px square into a 72 px box left the browser to do the resampling
...
SRC = ROOT / "docs" / "assets" / "author.png"
...
def main() -> None:
    ...
    args = ap.parse_args()
    if not SRC.exists():
        sys.exit(f"missing {SRC.relative_to(ROOT)} — run src/tools/make_previews.py first")
    src = Image.open(SRC)
    blob = build(src)
